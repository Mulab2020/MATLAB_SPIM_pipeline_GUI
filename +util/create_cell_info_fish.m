function cell_info=create_cell_info_fish(cell_info,celllabel,totcell,z)
...
dim=size(celllabel);
tot_inds=find(celllabel>0); yx=util.ind2sub2d(dim,tot_inds);    
[cell_yx(:,1) cell_yx(:,2) cell_yx(:,3)]=util.calc_cell_yx_mex(int32(celllabel(tot_inds)),int32(totcell),double(yx),int32(length(tot_inds)));
