...
void mexFunction( int Nreturned, mxArray *returned[], int Noperand, const mxArray *operand[] ){
    ...
    if (zrange[1] > zlen){zrange[1]=zlen;};
    
    
    int stacklen=wh[0]*wh[1]*zout;
    
    // read image files
    // minimized fread excution for communication between servers.
    unsigned short *stack;       
    returned[0] = mxCreateNumericMatrix(wh[0],wh[1]*zout,mxUINT16_CLASS,mxREAL);
    ...
    f=fopen(input_buf,"rb");
        _lseeki64(_fileno(f), singlelen*(__int64) (zrange[0]-1), SEEK_SET);         
