...
% Layout: Left sidebar (controls) + Right panel (preview + output)
    ...
    left_grid = uigridlayout(left_panel, [7 1], ...
                             'RowHeight', {45, 120, 85, 135, 35, '1x', 35}, ...
    ...
    step3_grid = uigridlayout(step3_panel, [5 1], ...
                              'RowHeight', {22, 22, 22, 25, 20}, ...
                              'Padding', 5, 'RowSpacing', 2);

    detrend_cb = uicheckbox(step3_grid, 'Text', 'Detrending', 'Value', true, ...
        'Tooltip', 'Apply sliding-window 15th percentile subtraction to remove slow baseline fluctuations (adds ~2 min per 1000 cells)');
    detrend_cb.Layout.Row = 1;

    dedup_cb = uicheckbox(step3_grid, 'Text', 'Remove double-counted cells', 'Value', false, ...
        'Tooltip', 'Correlate overlapping cells on adjacent z-planes and remove the redundant one (correlation threshold: 0.7)');
    dedup_cb.Layout.Row = 2;

    motionfilt_cb = uicheckbox(step3_grid, 'Text', 'Motion-based filtering', 'Value', false, ...
        'Tooltip', 'Remove cells located near grid points with motion > 1 pixel (requires motion_param.mat)');
    motionfilt_cb.Layout.Row = 3;
    ...
    function run_tcourse()
        ...
        try
            ...
            log_msg(sprintf('Options: detrending=%d, dedup=%d, motionfilt=%d', ...
                    tc_params.enable_detrending, tc_params.enable_remove_duplicates, ...
                    tc_params.enable_motion_filter));
    ...
    function batch_callback()
        ...
        try
            ...
            log_msg(sprintf('Batch options: run_motion=%d, use_gpu=%d, detrending=%d, dedup=%d, motionfilt=%d', ...
                    batch_opts.run_motion, batch_opts.use_gpu, ...
                    batch_opts.detrending, batch_opts.dedup, batch_opts.motionfilt));
